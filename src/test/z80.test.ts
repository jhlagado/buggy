...
describe('Z80 utilities', () => {
  ...
  it('parses listing addresses', () => {
    const listing = `0000   C3 39 00   JP RESET\n0900   31 00 FF   LD SP,STACK_TOP`;
    const info = parseListing(listing);
    assert.equal(info.lineToAddress.get(1), 0x0000);
    assert.equal(info.lineToAddress.get(2), 0x0900);
    assert.equal(info.addressToLine.get(0x0000), 1);
    assert.equal(info.addressToLine.get(0x0900), 2);
  });
